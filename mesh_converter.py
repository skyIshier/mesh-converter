...
"""
《光遇》.mesh 转 OBJ 通用脚本（修改版，支持 GUI 调用）
- 添加了 convert_mesh_to_obj 函数供外部调用
- 保留原有命令行/交互模式
"""
...
# ==================== 新增：供外部调用的转换函数 ====================
def convert_mesh_to_obj(input_path, output_dir, mode='hybrid'):
    """
    转换单个 .mesh 文件为 .obj
    参数:
        input_path: 输入的 .mesh 文件路径
        output_dir: 输出目录
        mode: 解析模式，可选 'hybrid', 'old', 'sky'
    返回:
        dict: 包含 success, vertex_count, face_count, parser, error 等信息的字典
    """
    lz4 = load_lz4()  # 每次调用加载一次（也可以作为全局变量，但简单起见这里加载）
    ...
    try:
        with open(input_path, 'rb') as f:
            if mode == 'old':
                vb, uvb, ib, parser_name = fallback_parse_all(f, lz4, input_path)
...
def main():
    ...
    for idx, mesh_file in enumerate(mesh_files, start=1):
        ...
        try:
            # 直接调用刚刚定义的转换函数
            conv_result = convert_mesh_to_obj(mesh_file, out_dir, mode)
            if conv_result['success']:
                result['status'] = 'success'
                result['vertex_count'] = conv_result['vertex_count']
                result['face_count'] = conv_result['face_count']
                result['parser'] = conv_result['parser']
                result['error'] = None
            else:
